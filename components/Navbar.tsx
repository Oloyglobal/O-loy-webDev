...
export default function Navbar() {
  ...
  // A link is "active" if the pathname matches exactly (for Home)
  // or starts with the href (for nested routes like /blog/post-slug)
  ...
  return (
    <>
      ...
      <nav
        ...
        style={{
          background: isScrolled ? 'rgba(10,5,26,0.97)' : 'rgba(8,3,20,0.92)',
        ...
        <div className="container-custom">
          <div className="flex items-center justify-between h-20">
                ...
                return (
                  <Link
                    ...
                    style={{
                      ...
                      color: active ? C.gold : C.txtSec,
                      letterSpacing: '0.15em',
                    }}
                    onMouseEnter={e => { if (!active) e.currentTarget.style.color = C.txtPri }}
                    onMouseLeave={e => { if (!active) e.currentTarget.style.color = C.txtSec }}
                  >
                    {link.name}

                    {/* ── Active indicator: solid gold underline + faint gold dot ── */}
                    {active ? (
                      <>
                        {/* Full-width solid gold bottom border */}
                        ...
                        {/* Small gold dot below the underline */}
                        <span className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full"
                          style={{ background: C.gold, boxShadow: `0 0 6px ${C.gold}` }} />
                      </>
                    ) : (
                      /* Hover-only underline (slide in from center) */
                      <span
            ...
            <div className="hidden lg:block">
              <Link
                href="/contact"
                className="inline-flex items-center px-7 py-3 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all duration-300 hover:scale-[1.04]"
                style={{
                  fontFamily: 'var(--font-body)',
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center min-h-screen px-8 relative z-10">
            ...
            {navLinks.map((link, index) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  className="flex items-center justify-center gap-3 py-3.5 font-bold transition-all duration-500"
                  style={{
                    fontFamily: 'var(--font-display)',
                    // Active: gold text + left gold bar; inactive: white
                    color: active ? C.gold : C.txtPri,
                    fontSize: '2rem',
                ...
                >
                  {/* Active indicator: gold left accent bar */}
                  {active && (
                    <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 rounded-full"
                      style={{ background: `linear-gradient(to bottom, ${C.gold}, ${C.goldLt})` }} />
                  )}
                  {link.name}
          ...
          <div className="mt-10 flex flex-col items-center gap-2 transition-all duration-500"
            ...
            <a href="tel:+2348069336270" className="flex items-center gap-2 text-sm"
              style={{ fontFamily: 'var(--font-body)', color: C.txtSec }}>
              <FaPhone size={10} style={{ color: C.gold }} />
              +234 8069336270
            </a>
            <a href="mailto:info@oloyglobal.com" className="text-xs"
